...
pub async fn chang_quota_state(
    ...
    if (qstr.state == "approve") || (qstr.state == "registe") || (qstr.state == "deined"){
        ...
        warn!("request state error:{:?} ,please input the correct state",qstr.state);
        return HttpResponse::Ok().json(ResponseBody::<()>::new_json_parse_error());
    }
    //修改数据表的值得状态
    match conn
        .query("UPDATE quota_admin SET extra = $1,state = $2,update_time = now() where aid = $3",
    &[&qstr.extra,&qstr.state,&qstr.aid]).await
    {
        Ok(row) => {
...
pub async fn get_quota_type(
    data: web::Data<Pool>,
    req: web::Json<StatesRequest>,
) -> impl Responder{
    //连接数据库句柄
    let conn = data.get().await.unwrap();
    let mut tatol:i64 = 0;   //总额度（额度管理表中状态为delivery的value和）
    let mut issued:i64 = 0;  //已发行额度（额度发行表中info信息字段和）
    let mut waitly:i64 = 0;   //待发放额度tatol-issued
    let mut recycle:i64 = 0;   //已回收额度 withdraw
    let mut select_tatol:Vec<Row> = Vec::new();
    let mut select_issued:Vec<Row> = Vec::new();
    let mut select_recycle:Vec<Row> = Vec::new();
    let type_delive = String::from("delivery");
    let type_with = String::from("withdraw");
//    let aid = Some(&req.aid);
//    if let Some(id) = aid{
    if req.aid.len() > 5{
        //总额度查询
        select_tatol = match conn
            .query("SELECT value from quota_admin where aid = $1 AND type = $2",
        &[&req.aid,&type_delive]).await{
            Ok(row) => {
        ...
        //已发额度查询
        select_issued = match conn
            .query("SELECT (issue_info->'t_obj'->'issue_info') from quota_delivery where aid = $1",
        &[&req.aid]).await{
            Ok(row) => {
                info!("electe success: {:?}", row);
                row
        ...
        //总额度查询
        select_tatol = match conn
            .query("SELECT value from quota_admin where type = $1",
        &[&type_delive]).await{
            Ok(row) => {
                info!("electe success: {:?}", row);
                row
        ...
        //已发额度查询
        select_issued = match conn
            .query("SELECT (issue_info->'t_obj'->'issue_info') from quota_delivery where $1 !='' ",
        &[&"aid"]).await{
            Ok(row) => {
                info!("electe success: {:?}", row);
                row
    ...
    //获取总额度
    for item in select_tatol.iter(){
        let value: i64 = item.get(0);
        tatol += value;
    }
    //获取已回收额度
    for item in select_recycle.iter(){
        let value: i64 = item.get(0);
        recycle += value;
    }
    //获取已发放额度总和
    for item in select_issued.iter(){
